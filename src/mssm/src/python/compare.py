...
def compare_CDL(
    ...
    """Computes the AIC difference and (optionally) performs an approximate GLRT on twice the
    difference in unpenalized likelihood between models ``model1`` and ``model2``
    (see Wood et al., 2016).

    For the GLRT to be appropriate ``model1`` should be set to the model containing more effects
    and ``model2`` should be a nested, simpler, variant of ``model1``.
    For the degrees of freedom for the test, the expected degrees of freedom (EDF) of each model
    are used (i.e., this is the conditional test discussed in Wood (2017: 6.12.4)).
    The difference between the models in EDF serves as DoF for computing the Chi-Square statistic.
    In addition, ``correct_t1`` should be set to True, when computing the GLRT.

    To get the AIC for each model, 2*edf is added to twice the negative (conditional) likelihood
    (see Wood et al., 2016).

    By default (``correct_V=True``), ``mssm`` will attempt to correct the edf for uncertainty in
    the estimated :math:`\\lambda` parameters. Which correction is computed depends on the
    choice for the ``grid`` argument. **Approximately** the analytic solution for the correction
    proposed by Wood, Pya, & Säfken (2016) is computed  when ``grid='JJJ1'`` (the default) - which
    is exact for strictly Gaussian and some canonical Generalized additive models. This is too
    costly for very large sparse multi-level models and not exact for more generic models. The MC
    based alternative available via ``grid = 'JJJ2'`` addresses the first problem (**Important**,
    set: ``use_importance_weights=False`` and ``only_expected_edf=True``.). The second MC
    based-alternative available via ``grid_type = 'JJJ3'`` is most appropriate for more generic
    models (The ``prior`` argument can be used to specify any prior to be placed on
    :math:`\\boldsymbol{\\rho}` also you will need to set: ``use_importance_weights=True`` and
    ``only_expected_edf=False``).
    For more details consult the :func:`mssm.src.python.utils.correct_VB` function, the examples
    below, and Krause et al. (submitted).

    In case any of those correction strategies is too expensive, it might be better to rely on
    hypothesis tests for individual smooths, confidence intervals, and penalty-based selection
    approaches instead (see Marra & Wood, 2011 for details on the latter).

    In case ``correct_t1=True`` the EDF will be set to the (smoothness uncertainty corrected in case
    ``correct_V=True``) smoothness bias corrected exprected degrees of freedom (t1 in section 6.1.2
    of Wood, 2017), for the GLRT (based on recomendation given in section 6.12.4 in Wood, 2017).
    The AIC (Wood, 2017) of both models will **still be based on the regular (smoothness uncertainty
    corrected) edf**.

    The computation here is different to the one performed by the ``compareML`` function in the
    R-package ``itsadug`` - which rather performs a version of the marginal GLRT
    (also discussed in Wood, 2017: 6.12.4) - and more similar to the ``anova.gam`` implementation
    provided by ``mgcv`` (particularly if ``grid='JJJ1'). The returned p-value is approximate -
    very **very** much so if ``correct_V=False`` (this should really never be done). Also, the GLRT
    ...
    References:
     ...
     - ``compareML`` function from ``itsadug`` R-package: \
        https://rdrr.io/cran/itsadug/man/compareML.html
     - ``anova.gam`` function from ``mgcv``, see: \
        https://www.rdocumentation.org/packages/mgcv/versions/1.9-1/topics/anova.gam
    ...
    :param correct_t1: Whether or not to also correct the smoothness bias corrected edf for
        smoothness uncertainty. Defaults to None - meaning that ``mssm`` will select an appropriate
        value.
    :type correct_t1: bool | None, optional
    :param perform_GLRT: Whether to perform both a GLRT and to compute the AIC or to only compute
        the AIC. Defaults to True.
    ...
    :param grid: How to compute the smoothness uncertainty correction, defaults to None - meaning
        that ``mssm`` will select an appropriate value.
    :type grid: str | None, optional
    :param a: Any of the :math:`\\lambda` estimates obtained from ``model`` (used to define the
        mean for the posterior of :math:`\\boldsymbol{\\rho}|y \\sim \
        N(log(\\hat{\\boldsymbol{\\rho}}),\\mathbf{V}_{\\boldsymbol{\\rho}})` used to sample
        ``nR`` candidates) which are smaller than this are set to this value as well,
        defaults to 1e-7 the minimum possible estimate
    :type a: float, optional
    :param b: Any of the :math:`\\lambda` estimates obtained from ``model`` (used to define the
        mean for the posterior of :math:`\\boldsymbol{\\rho}|y \\sim \
        N(log(\\hat{\\boldsymbol{\\rho}}),\\mathbf{V}_{\\boldsymbol{\\rho}})` used to sample
        ``nR`` candidates) which are larger than this are set to this value as well, defaults to
        1e7 the maximum possible estimate
    :type b: float, optional
    :param df: Degrees of freedom used for the multivariate t distribution used to sample/propose
        the next set of candidates. Setting this to ``np.inf`` means a multivariate normal is used
        for sampling, defaults to 40
    ...
    :param only_expected_edf: Whether to compute edf. by explicitly forming covariance matrix
        (``only_expected_edf=False``) or not. The latter is much more efficient for sparse models
        at the cost of access to the covariance matrix and the ability to compute an upper bound on
        the smoothness uncertainty corrected edf. Only makes sense when ``grid_type!='JJJ1'``.
        Defaults to None - meaning that ``mssm`` will select an appropriate value.
    :type only_expected_edf: bool|None, optional
    :param Vp_fidiff: Whether to rely on a finite difference approximation to compute
        :math:`\\mathbf{V}_{\\boldsymbol{\\rho}}` or on a PQL approximation. The latter is exact for
        Gaussian and canonical GAMs and far cheaper if many penalties are to be estimated.
        Defaults to False (PQL approximation)
    ...
    :param use_importance_weights: Whether to rely importance weights to compute the numerical
        ...
        :math:`\\mathbf{V}_{\\boldsymbol{\\rho}}` - the latter assumes that the unconditional
        posterior is normal. Defaults to None - meaning that ``mssm`` will select an appropriate
        value.
    ...
    :param recompute_H: Whether or not to re-compute the Hessian of the log-likelihood at an
        ...
        computing the (uncertainty/bias corrected) edf. Defaults to None - meaning that ``mssm``
        will select an appropriate value.
    :type recompute_H: bool | None, optional
    :param compute_Vcc: Whether to compute the second correction term when `grid='JJJ1'` (or when
        computing the lower-bound for the remaining grids) or only the first one. In contrast to the
        second one, the first correction term is substantially cheaper to compute - so setting this
        to False for larger models will speed up the correction considerably. Defaults to None -
        meaning that ``mssm`` will select an appropriate value.
    ...
    :param bfgs_options: An optional dictionary holding arguments that should be passed on to the
        call of :func:`scipy.optimize.minimize` if ``method=='qEFS'``. If none are provided, the
        ``gtol`` argument will be initialized to ``conv_tol``. Note also, that in any case the
        ``maxiter`` argument is automatically set to ``max_inner``. Defaults to None.
    :type bfgs_options: dict,optional
    :raises ValueError: If both models are from different families.
    :raises ValueError: If ``perform_GLRT=True`` and ``model1`` has fewer coef than ``model2`` -
        i.e., ``model1`` has to be the notationally more complex one.
    :return: A dictionary with outcomes of all tests. Key ``H1`` will be a bool indicating whether
        Null hypothesis was rejected or not, ``p`` will be the p-value, ``test_stat`` will be the
        test statistic used, ``Res. DOF`` will be the degrees of freedom used by the test, ``aic1``
        and ``aic2`` will be the aic scores for both models.
    :rtype: dict
    """
    ...
    if grid is None:
        if isinstance(model1.family, Gaussian) and isinstance(
            ...
            if len(model1.coef) > 2000 and n_c > 5 and perform_GLRT is False:
                grid = "JJJ2"
    ...
    if only_expected_edf is None:
        if (
            ...
            and perform_GLRT is False
            and correct_t1 is False
        ):
    ...
    # Collect total DOF for uncertainty in \\lambda using correction proposed by
    # Greven & Scheipl (2016)
    aic_DOF1 = None
    aic_DOF2 = None
    if correct_V:
        ...
        if correct_t1:
            # Section 6.12.4 suggests replacing t (edf) with t1 (2*t - (F@F).trace()) with
            # F=(X.T@X+S_\\llambda)^{-1}@X.T@X for GLRT - with the latter also being corrected for
            # uncertainty in lambda. However, Wood et al., (2016) suggest that the aic should be
            # computed based on t - so some book-keeping is necessary.
            aic_DOF1 = DOF1
            aic_DOF2 = DOF2
            DOF1 = DOF12
            DOF2 = DOF22
        ...
        if correct_t1:
            # Compute uncertainty un-corrected but smoothness bias corrected edf
            # (t1 in section 6.1.2 of Wood, 2017)
    ...
    # 1) LLK1 < LLK2, DOF1 < DOF2; This is a valid test, essentially model2 turns out to be the
    #    more complicated one.
    # 2) LLK1 < LLK2, DOF1 > DOF2; This makes no sense. Model 1 - the more complex one - has worse
    #    llk but more DOF.
    # 3) LLK1 > LLK2, DOF1 < DOF2; Notationally correct: model1 should after all be more complex.
    #    But in terms of edf makes little sense (as pointed out by Wood, 2017).
    # 4) LLK1 > LLK2, DOF1 > DOF2; Valid, inverse of case 1.

    # Personally, I think cases 2 & 3 should both return NAs for p-values.. But anova.gam for mgcv
    # returns a p-value for case 3 so we will do the same here
    ...
    if (
        ...
        if llk1 > llk2 and DOF1 < DOF2:
            warnings.warn(
                (
                    "Model with more coefficients has higher likelihood but lower expected degrees "
                    "of freedom. Interpret results with caution."
                )
            )
