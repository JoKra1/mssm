...
def sample_MVN(
    ...
    mu: int | np.ndarray,
    scale: float,
    P: scp.sparse.csc_array | None,
    L: scp.sparse.csc_array | None,
    LI: scp.sparse.csc_array | None = None,
    ...
    # Sample from N(0,1)
    z = scp.stats.norm.rvs(size=Cs.shape[1] * n, random_state=seed).reshape(
        Cs.shape[1], n
    )
    ...
    if isinstance(model.family, Family):  # GAMM case
        ...
        if model.coef is not None:
            ...
            split_idx = np.ndarray.flatten(
                np.split(np.arange(len(model.coef)), model.coef_split_idx)[par]
            )
    ...
    if coef is None:
        ...
        if len(form.file_paths) != 0:
            raise NotImplementedError(
                "Cannot return p-value for parametric terms if X.T@X was formed iteratively."
            )
        ...
        if isinstance(model.family, Family) and model.family.twopar:
            ps = 1 - scp.stats.t.cdf(
                ts, df=len(form.y_flat[form.NOT_NA_flat]) - form.n_coef
            )
        else:
...
def print_smooth_terms(
    ...
    if ps is not None and Trs is None:
        raise ValueError(
            "To display p-values, both ``ps`` and ``Trs`` needs to be provided."
        )
    ...
    term_names = np.array(form.get_term_names())
    smooth_names = [
        *term_names[form.get_smooth_term_idx()],
            ...
            if sterm.by is not None and sterm.id is None:
                for li in range(len(form.get_factor_levels()[sterm.by])):
                    ...
                    e_str = (
                        ...
                        + f": {coding_factors[sterm.by][li]}; edf: {t_edf}"
                    )
                    if t_edf < pen_cutoff:
            ...
            if pen.start_index > prev_start_idx:
                ...
                if pen.dist_param == par:  # Collect only those belonging to this par
                    if edf1:
                        term_edf.append(
                            min(
                                model.term_edf1[prev_edf_idx],
                                pen.rep_sj * pen.S_J.shape[1],
                            )
                        )
                    else:
                        term_edf.append(
                            min(
                                model.term_edf[prev_edf_idx],
                                pen.rep_sj * pen.S_J.shape[1],
                            )
                        )
            ...
            for _ in enumerator:
                ...
                if k > 0:
                    ...
                    if v == 0:  # Integer case - standard F-test
                        ...
                        S = np.diag(
                            [1 / lam if lami < k else 0 for lami, lam in enumerate(s)]
                        )
                        ...
                        Lb = np.array(
                            [[np.power(s[k - 1], -0.5), 0], [0, np.power(s[k], -0.5)]]
                        )
                        ...
                        if isinstance(model.family, Family) and model.family.twopar:
                            ...
                            p1, code1, _ = davies.daviesQF(
                                np.array([*lb, -Tr1 / rs_df]),
                                nc,
                                n,
                                0,
                                0,
                            ...
                            p2, code2, _ = davies.daviesQF(
                                np.array([*lb, -Tr2 / rs_df]),
                                nc,
                                n,
                                0,
                                0,
                            ...
                            if code1 > 0 or code2 > 0 or force_approx:
                                ...
                                if code1 != 2 or code2 != 2:
                                    ...
                                    p2 = 1 - scp.stats.beta.cdf(
                                        (r * Tr2 / rs_df) / (1 + (r * Tr2 / rs_df)),
                                        a=r / 2,
                                        b=rs_df / 2,
                                    )
                            ...
                            # cast to np.array
                            nc = np.array(
                                [0.0 for _ in range(len(n))], dtype=np.float64
                            )
                            n = np.array(n)
                            lb = np.array(lb)

                            p1, code1, _ = davies.daviesQF(
                                lb, nc, n, Tr1, 0, 2e-5, len(n), 20000
                            )
                            p2, code2, _ = davies.daviesQF(
                                lb, nc, n, Tr2, 0, 2e-5, len(n), 20000
                            )
                            p1 = 1 - min(p1, 1)
                            p2 = 1 - min(p2, 1)
...
def adjust_CI(
    ...
    n_ps: int,
    b: np.ndarray,
    predi_mat: scp.sparse.csc_array,
    use_terms: list[int] | None,
    alpha: float,
    seed: int | None,
    par: int = 0,
) -> np.ndarray:
...
def compute_reml_candidate_GAMM(
    ...
    init_eta: np.ndarray | None = None,
    method: str = "Chol",
    compute_inv: bool = False,
    origNH: float | None = None,
) -> tuple[
    float,
    ...
    S_emb, _, S_root, _ = compute_S_emb_pinv_det(
        X.shape[1], penalties, "svd", method != "Chol"
    )
...
def compute_REML_candidate_GSMM(
    ...
    try:
        ...
        total_edf, _, _ = calculate_edf(
            None, None, LV, penalties, lgdetDs, n_coef, n_c, None, S_emb
        )
    ...
    **Important**: the dimension of the output depend on the shape of ``coef``. If ``coef`` is
    ...
    for Si, S_rep in enumerate(S_reps):
        ...
        if code == 0:
            ...
            ldetSI = (
                np.log(np.abs(R.diagonal())).sum() * Sj_reps[SJ_term_idx[Si][0]].rep_sj
            )
    ...
    if code != 0:
        raise ValueError(
            f"Failed to compute REML for lambda: {[pen.lam for pen in penalties]}."
        )
...
def estimateVp(
    ...
    **bfgs_options,
    ...
    if grid_type == "JJJ1" or grid_type == "JJJ2":
        ...
        ep = np.log(
            np.array([pen.lam for pen in model.overall_penalties]).reshape(-1, 1)
        )
        ...
        if Vp_fidiff:
            ...
            def reml_wrapper(rho):
                ...
                if isinstance(family, Family):
                    reml, _, _, _, _, _, _, _ = compute_reml_candidate_GAMM(
                        family, y, X, rPen, n_c, model.offset, model.preds[0], method
                    )
                else:
                    reml, _, _, _, _, _ = compute_REML_candidate_GSMM(
                        family,
                        y,
                        Xs,
                        rPen,
            ...
            nHp = scp.differentiate.hessian(
                lambda r: np.apply_along_axis(reml_wrapper, axis=0, arr=r),
                ep.flatten(),
                order=4,
                maxiter=3,
            ...
            if -3 in nHp.status.flatten():
                raise ValueError(
                    "Finite differencing step failed. Set ``Vp_fidiff=False``."
                )
            nHp = nHp.ddf
    ...
    # fmt: off
    ep = np.log(
        np.array([min(b, max(a, pen.lam)) for pen in model.overall_penalties]).reshape(-1, 1)
    )
    # fmt: on
    # print(ep)
    ...
    if n_c > 1:  # Parallelize grid search
        ...
        p_sample = scp.stats.multivariate_t.rvs(
        ...
        if len(np.ndarray.flatten(ep)) == 1:  # Single lambda parameter in model
            ...
            if (
                n_est == 1
            ):  # and single sample (n_est==1) - so p_sample needs to be shape (1,1)
                p_sample = np.array([p_sample])

            p_sample = p_sample.reshape(
                n_est, 1
            )  # p_sample needs to be shape (n_est,1)

        elif (
            n_est == 1
        ):  # multiple lambdas - so p_sample needs to be shape (1,n_lambda)
        ...
        if (
            ...
            with (
                ...
                # XX
                _, _, _, dataXX, indptrXX, indicesXX = map_csc_to_eigen(
                    (X.T @ X).tocsc()
                )
                ...
                dat_sharedXX = np.ndarray(
                    shape_datXX, dtype=np.double, buffer=dat_memXX.buf
                )
                dat_sharedXX[:] = dataXX[:]

                ptr_memXX = manager.SharedMemory(indptrXX.nbytes)
                ptr_sharedXX = np.ndarray(
                    shape_ptrXX, dtype=np.int64, buffer=ptr_memXX.buf
                )
                ptr_sharedXX[:] = indptrXX[:]

                idx_memXX = manager.SharedMemory(indicesXX.nbytes)
                idx_sharedXX = np.ndarray(
                    shape_datXX, dtype=np.int64, buffer=idx_memXX.buf
                )
                ...
                args = zip(
                    ...
                    repeat(ptr_mem.name),
                    repeat(idx_mem.name),
                    repeat(dat_memXX.name),
                    repeat(ptr_memXX.name),
                    repeat(idx_memXX.name),
                    repeat(shape_y),
                    repeat(shape_dat),
                    repeat(shape_ptr),
                    repeat(shape_datXX),
            ...
            if isinstance(family, Family):
                ...
                with mp.Pool(processes=n_c) as pool:
                    (
                        ...
                        sample_edfs,
                        sample_llks,
                    ) = zip(*pool.starmap(compute_reml_candidate_GAMM, args))
            else:
                origNH = None
                args = zip(
                    repeat(family),
                    ...
                    rPens,
                    repeat(init_coef),
                    repeat(len(init_coef)),
                    repeat(model.coef_split_idx),
                    repeat(method),
                    repeat(1e-7),
                    repeat(1),
                    repeat(bfgs_options),
        ...
        for ps in p_sample:
            ...
            if isinstance(family, Family):
                reml, Linv, LP, Pr, coef, scale, edf, llk = compute_reml_candidate_GAMM(
                    family,
                    y,
                    X,
                    rPen,
                ...
                try:
                    reml, V, _, coef, edf, llk = compute_REML_candidate_GSMM(
                        family,
                        y,
                        Xs,
                        rPen,
    ...
    if use_importance_weights and prior is None:
        ...
        use_importance_weights and prior is not None
    ):  # Standard importance weights (e.g., Branchini & Elvira, 2024)
        logp = prior.logpdf(np.log(rGrid))
        q = scp.stats.multivariate_t(
            loc=np.ndarray.flatten(ep), shape=Vpr, df=df, allow_singular=True
        )
...
def _compute_VB_corr_terms_MP(
    ...
    XX = scp.sparse.csc_array(
        (dataXX, indicesXX, indptrXX), shape=(cols, cols), copy=False
    )
    ...
    if code != 0:
        raise ValueError(
            "Forming coefficients for specified penalties was not possible."
        )
...
def compute_Vp_WPS(
    ...
    Sj_reps, _, S_inv_rp, _, S_reps, SJ_term_idx, SJ_idx, S_coefs, Q_reps, Mp = reparam(
        None, penalties, None, option=4, form_inverse=2
    )
    ...
    for peni in range(len(penalties)):
        ...
        for penj in range(peni, len(penalties)):
            ...
            t2 = (
                ...
                / (scale)
                * coef.T
                @ penalties[peni].S_J_emb
                @ dBetadRhos[:, [penj]]
            ) + (
                penalties[penj].lam
                / (scale)
                * coef.T
                @ penalties[penj].S_J_emb
                @ dBetadRhos[:, [peni]]
            )
            ...
            if gamma or (penalties[peni].start_index == penalties[penj].start_index):
                t3 = (
                    ...
                    * penalties[peni].lam
                    * penalties[penj].lam
                    * penalties[peni].rep_sj
                    * (
                        S_inv_rp
                        @ Sj_reps[penj].S_J_emb
                        @ S_inv_rp
                        @ Sj_reps[peni].S_J_emb
                    ).trace()
                )
                ...
                if gamma:
                    t3 += (
                        penalties[peni].lam
                        * penalties[peni].rep_sj
                        * (S_inv_rp @ Sj_reps[peni].S_J_emb).trace()
                    )
    ...
    **Note:** If you set ``only_expected_edf=True``, only the last two output arguments will be
    ...
    if grid_type == "JJJ1" or grid_type == "JJJ2" or grid_type == "JJJ3":
        ...
        if Vp_fidiff:
            ...
            # Take PQL approximation instead
            Vp, Vpr, Vr, Vrr, _, dBetadRhos = compute_Vp_WPS(
                model.lvi,
                model.hessian,
                S_emb,
                model.overall_penalties,
    ...
    if grid_type != "JJJ1":
        ...
        ep = np.log(
            np.array(
                [min(b, max(a, pen.lam)) for pen in model.overall_penalties]
            ).reshape(-1, 1)
        )
        # print(ep)
        ...
        if n_c > 1:  # Parallelize grid search
            ...
            for ps in p_sample:
                ...
                if isinstance(family, Family):
                    reml, Linv, LP, Pr, coef, scale, edf, llk = (
    ...
    if grid_type != "JJJ1":
        ...
        if recompute_H and (only_expected_edf is False):
            ...
            if isinstance(family, Family):
                ...
                if isinstance(family, Gaussian) and isinstance(
                    ...
                    if inval_check:
                        ...
                        Wr_fix = scp.sparse.spdiags(
                            [np.sqrt(np.ndarray.flatten(w))], [0]
                        )
                    else:
                        Wr_fix = Wr
                ...
                # Re-compute scale
                _, _, _, _, _, scale = update_scale_edf(
                    y,
                    z,
                    eta,
                    Wr,
