...
class f(GammTerm):
    ...
    def __init__(
        ...
        if te == True and len(variables) == 1:
            raise ValueError(
                "``te`` can only be set to true in case multiple variables are provided via ``variables``."
            )
    ...
    def absorb_repara(self, rpidx, X, cov):
        ...
        if len(self.RP) == 0:
            warnings.warn(
                "RP method called for term that should not be re-parameterized. Skipping RP attempt."
            )
            return
    ...
    def build_penalty(
        ...
        if self.should_rp == 1:
            ...
            if len(vars) > 1:
                raise ValueError(
                    f"Demmler and Reinsch reparameterisation only supported for univariate smooth terms - but requested for term {ti}."
                )
        ...
        if len(vars) > 1:
            ...
            if self.te and self.is_identifiable:
                constraint = self.Z[
                    0
                ]  # Zero-index because a single set of identifiability constraints exists: one for the entire Tp basis.
            else:
                constraint = None
            ...
            if self.id is not None:
                ...
                if self.drop_coef is None:
                    ...
                    lTerm.S_J = lTerm.S_J_emb[
                        lTerm.start_index : cur_pen_idx, lTerm.start_index : cur_pen_idx
                    ]
            ...
            if self.is_identifiable and self.te == False:
                if self.Z[vi].type == ConstType.QR:
                    ...
                    matrix_term_v = np.diff(
                        np.concatenate(
                            (
                                matrix_term_v[:, self.Z[vi].Z : matrix_term_v.shape[1]],
        ...
        if self.is_identifiable and self.te:
            if self.Z[0].type == ConstType.QR:
                ...
                matrix_term = np.diff(
                    np.concatenate(
                        (
                            matrix_term[:, self.Z[0].Z : matrix_term.shape[1]],
...
def build_ir_smooth_series(
    ...
    if irsterm.by is not None:
        ...
        if len(np.unique(by_cov)) > 1:
            raise ValueError(
                f"By-variable {irsterm.by} has varying levels on series level. This should not be the case."
            )
