...
class HSMMFamily(GSMMFamily):
    ...
    def series_Tpi(
        ...
        :param series: Series for which to compute transition matrices.
        :type series: int
        :param coef: The current coefficient estimate (as np.array of shape (-1,1) - so it must not
            be flattened!).
        :type coef: np.ndarray
    ...
    def predict_series(
        ...
        coefficients ``coef`` for a single series.
        ...
        :type n_samples: int
        :param scale: Scale parameter to use for HMP signal prediction.
        :type scale: float
        :return: two numpy arrays for single series. First numpy array is
        ...
        for sample in range(n_samples):
            ...
            dur = np_gen.choice(d_vals, p=ds[:, state, 0] if tvdtpi else ds[:, state])
        ...
        if n_cores == 1:
            ...
            for series in range(n_series):
                with warnings.catch_warnings():  # Supress warnings
                    ...
                    e_predss, s_predss = self.predict_series(
                        series,
                        coef,
                        coef_split_idx,
                        split_Ys[series],
            ...
            with warnings.catch_warnings():  # Supress warnings
                warnings.simplefilter("ignore")
                with mp.Pool(processes=n_cores) as pool:
                    e_preds, s_preds = zip(*pool.starmap(self.predict_series, args))
    ...
    def decode_viterbi(
        ...
        if n_cores == 1:
            ...
            for series in range(n_series):
                with warnings.catch_warnings():  # Supress warnings
                    warnings.simplefilter("ignore")
                    edss, statess = self.series_viterbi(
                        series,
                        coef,
                        coef_split_idx,
                        split_Ys[series],
    ...
    def sample_posterior_series(
        ...
        """Samples the posterior of state sequences for a single series given parameter estimates
        and data.
    ...
    def sample_posterior_states(
        ...
        if n_cores == 1:
            ...
            for series in range(n_series):
                with warnings.catch_warnings():  # Supress warnings
                    warnings.simplefilter("ignore")
                edss, statess = self.sample_posterior_series(
                    series,
                    coef,
                    coef_split_idx,
                    split_Ys[series],
    ...
    def series_resid(
        ...
        See :func:`get_resid` for details.
    ...
    def series_grad(
                ...
                for j in range(n_S):
                    ...
                    if tvdtpi:
                        ...
                        for tr in state_vals:
                            d1etaT = [
                                jtFam.dpars(
                                    np.array([tr for _ in range(n_T)]).reshape(-1, 1),
                                    *mustj,
                                    index=d1i,
                                    order=DerivOrder.d1,
                                )
                    ...
                    if tvdtpi:
                        ...
                        mustj /= mu_firstT[None, :]
                        # (j,n_S-1,n_T)
                        Ts[j, Tj_idx, :] = np.insert(mustj, 0, 1 / mu_firstT, axis=0)
                    else:
        ...
        if len(shared_pars) > 0:
            ...
            ind_coef_idx_flat = np.array(
                [cidx for cidx in range(total_coef) if cidx not in shared_coef_idx_flat]
            )
