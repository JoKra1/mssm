...
class Test_model_comparison3_hard:
    ...
    sim_fit_formula_sd = Formula(lhs("y"), [i()], data=sim_fit_dat, print_warn=False)

    sim_fit_model = GAMMLSS(
        [sim_fit_formula, copy.deepcopy(sim_fit_formula_sd)],
    ...
    sim_fit_model2 = GAMMLSS(
        [sim_fit_formula2, copy.deepcopy(sim_fit_formula_sd)],
        family=GAMMALS([LOG(), LOGb(-0.01)]),
    )
    sim_fit_model2.fit(**test_kwargs_model)
...
class Test_model_comparisons4:
    sim_fit_dat = sim3(n=500, scale=2, c=0.0, family=Gaussian(), seed=2)
    ...
    sim_fit_model = GAMM(sim_fit_formula, ScaledT(Identity(), theta=None))
...
class Test_Vb_corrections:
    ...
    Vp1, _, _, _, _, _ = estimateVp(model, grid_type="JJJ1", seed=20)
    ...
    def test_Vp1(self):
        assert np.allclose(
            np.round(self.Vp1, decimals=3),
            np.array(
                [
                    [1.134000e00, -7.000000e-03, 7.000000e-03, -1.050000e-01],
                    [-7.000000e-03, 8.970000e-01, 1.600000e-02, -7.000000e-03],
                    [7.000000e-03, 1.600000e-02, 4.470000e-01, 9.600000e-02],
                    [-1.050000e-01, -7.000000e-03, 9.600000e-02, 5.989569e03],
                ]
            ),
        )
